...
int main(int argc, char *argv[])
{
	...
	// ******************** NbPoints **************
	ParameterDelaults.NbPoints = 4;
	char Str_NbPoints[] = "-NbPoints";
	//********************* Parameter y,z,w *************
	ParameterDelaults.start = -10.0f;
	char Str_Q_start[] = "-Q_start";
	ParameterDelaults.end = 10.0f;
	...
	strcpy(NameFile_statG, NameFile);
	strcat(NameFile_statG, "G.stat");
	...
	for(POWER=2.0f; POWER<=10.0f; POWER+=0.1f)
	{
		...
		file.open(NameFile_statG);
		file << "Statistiques Globales: \n";
		file << "				X = " << Stat.X << "\n";
